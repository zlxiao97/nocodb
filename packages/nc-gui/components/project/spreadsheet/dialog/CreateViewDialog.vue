  ...
  <v-dialog v-model="localState" max-width="500">
    <v-card class="elevation-20">
      ...
      <v-card-text class="pt-4 pl-4">
        <p class="headline">
          {{ $t("general.create") }}
          <span class="text-capitalize">{{ typeAlias }}</span>
          {{ $t("objects.view") }}
        </p>
        <v-form ref="form" v-model="valid" @submit.prevent="createView">
          ...
          <v-text-field
            ...
            :rules="[
              ...
              (v) =>
                viewsList.every((v1) => (v1.alias || v1.title) !== v) ||
                'View name should be unique',
            ]"
            autofocus
          />
        </v-form>
      ...
      try {
        ...
        switch (this.show_as) {
          ...
          case ViewTypes.CUSTOM:
            data = await this.$api.dbView.customCreate(this.meta.id, {
              title: this.view_name,
              copy_from_id: this.selectedViewId,
            });
            break;
